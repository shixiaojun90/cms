...
//    }
	...
	final public  function tabPage($rowCount,$current,$sort='id desc',$like=null,$where=null){
        ...
        if(is_array($sort)){
			...
			if($like){
				$tsql='(select t.id from (select '.TB_PK.' from `'. $this->table. '` where '.$str.($where!=null?' and '.$where:' ').' order by '.$sort.' limit '.$startCount.','.$rowCount.')as t)'.' order by  '.$sort;
			}else{
				$tsql='(select t.id from (select '.TB_PK.' from `'. $this->table.'`'. ($where!=null?' where '.$where:' ').' order by '.$sort. ' limit '.$startCount.','.$rowCount.')as t)'.' order by  '.$sort;
			}
		}
