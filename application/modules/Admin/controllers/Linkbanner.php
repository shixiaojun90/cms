...
class LinkbannerController extends BasicController {
    private function init(){
        Yaf_Registry::get('adminPlugin')->checkLogin();
        Yaf_Registry::get('PermissionPlugin')->checkPermission();
        $this->m_admin=$this->load('User');
        $this->m_linkbanner = $this->load('linkbanner');
    }
    ...
    public function tabPageAction(){
        try{
            //查询
			$sort='id desc';
            $data=$this->m_linkbanner->getAppArtListByPage($_POST['rowCount'],$_POST['current'],$sort,$_POST['searchPhrase'],null);
    ...
    public function getArticleByIdAction(){
        try{
            //查询
            $data=$this->m_linkbanner->getArticlebyId($_GET['id']);
            Helper::response('0',$data);
        }catch (Exception $ex){
            Helper::response('1006',$ex);
        }
    ...
    public function addAction(){
        try{
            ...
            $data=$this->m_linkbanner->Insert($ret);
    ...
    public function deleteAction(){
        try{
            $result = $this->m_linkbanner->DeleteByID($_POST['id']);
    ...
    public function updateByIdAction(){
        try{
            ...
            $data=$this->m_linkbanner->UpdateByID($ret,$_POST['id']);
            Helper::response('0',$data);
        }catch (Exception $ex){
            Helper::response('1006',$ex);
        }
