...
class ArticleinfoController extends BasicController {
    ...
    public function tabPageAction(){
        try{
            //查询
			$sort='id desc';
            $data=$this->m_articleinfo->getAppArtListByPage($_POST['rowCount'],$_POST['current'],$sort,$_POST['searchPhrase'],null);
    ...
    public function addAction(){
        try{
            //添加
            $id=$this->m_articleinfo->Insert(array('creater'=>$_SESSION['user']['id']));
            $ret=$_POST;
            $ret['c_time']=time();
            $ret['creater']=$_SESSION['user']['id'];
            $path='upload/article/'.$id.'/avatar/';
            if(isset($ret['file'])){
                unset($ret['file']);
            }else{
                ...
                $ret['icon']='https://'.$_SERVER['HTTP_HOST'].'/'.$path.$up->upload($newFileName)['img'];
            }
            if(isset($ret['files'])){
                unset($ret['files']);
            }
            $data=$this->m_articleinfo->UpdateByID($ret,$id);
    ...
    public function deleteAction(){
        try{
            $result = $this->m_articleinfo->DeleteByID($_POST['id']);
    ...
    public function updateByIdAction(){
        try{
            ...
            $path='upload/article/'.$_POST['id'].'/avatar/';
            if(isset($ret['file'])){
                unset($ret['file']);
            }else{
                $up = new Upload($_FILES['file'], $path);
            ...
            $data=$this->m_articleinfo->UpdateByID($ret,$_POST['id']);
