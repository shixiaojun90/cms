...
class ZhaacController extends BasicController {
    ...
    public function tabPageAction(){
        try{
            ...
            $data=$this->m_zhaac->getAppArtListByPage($_POST['rowCount'],$_POST['current'],$sort,$_POST['searchPhrase'],null);
    ...
    public function addAction(){
        try{
			...
			if(empty($zharr['zhnews']['name'])){
				$error['code']='1002';
				$error['msg']='中文名称不能为空';
				Helper::response($error);
			}elseif(empty($zharr['zhnews']['content'])){
				$error['code']='1002';
				$error['msg']='中文内容不能为空';
				Helper::response($error);
			}elseif(empty($enarr['ennews']['name'])){
				$error['code']='1002';
				$error['msg']='英文名称不能为空';
				Helper::response($error);
			}elseif(empty($enarr['ennews']['content'])){
				$error['code']='1002';
				$error['msg']='英文内容不能为空';
				Helper::response($error);
			}else{
    ...
    public function updateByIdAction(){
        try{
			...
			if(empty($zharr['zhnews']['name'])){
				...
				$auditid=$this->m_audit->Where(array('id'=>$zharr['zhnews']['id'],'audit_type'=>0))->SelectOne();
				if(!empty($auditid)){
					if(!empty($_POST['zhnews']) && !empty($_POST['ennews'])){
						$zharr['zhnews']['u_time']=time();
						$enarr['ennews']['u_time']=time();
						...
						$kaaac=$this->m_kaaac->getArticlebyId($auditid['id']);
						if(empty($kaaac)){
							//韩文kaaac表
							$kaarr['kanews']['c_time']=time();
