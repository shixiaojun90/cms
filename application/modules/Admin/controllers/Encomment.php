 ...
 * Time: 上午10:42
 */
class EncommentController extends BasicController {
    ...
    public function tabPageAction(){
        try{
            //查询
			$sort='id DESC';
            $data=$this->m_comment->getArtListByPage($_POST['rowCount'],$_POST['current'],$sort,$_POST['searchPhrase'],'(language_id=2)');
			
            foreach ($data['data'] as $key=>$val){
                $data['data'][$key]['username']=$this->m_register->getUserById($val['cuid'])['username'];
                $data['data'][$key]['article_id']=$this->m_enaac->getArticlebyId($val['article_id'])['name'];
