...
class NewaacController extends BasicController {
    ...
    public function tabPageAction(){
        try{
            //查询
			$sort='id desc';
            $data=$this->m_newaac->getArtListByPage($_POST['rowCount'],$_POST['current'],$sort,$_POST['searchPhrase'],null);
            foreach ($data['data'] as $key=>$val){
                $data['data'][$key]['moneyadd']=$this->m_aacqq->getArticlebyqq($val['qq'])['moneyadd'];
            }
    ...
    public function deleteAction(){
        try{
            $result = $this->m_newaac->DeleteByID($_POST['id']);
	...
	public function excelAction($ex='2007'){
        ...
        $links_list=$this->m_newaac->excelqq();
		$aacqq=$this->m_aacqq->excelqq();
		...
		$objExcel->getActiveSheet()->setCellValue('g1', "$k6");
		$objExcel->getActiveSheet()->setCellValue('i1', "$k7");
		$i=0;
		foreach($links_list as $k=>$v) {
			$u1=$i+2;
			$logins=$v['logins'].'次';
			 /*----------写入内容-------------*/
			$moneyadd=$this->m_aacqq->getArticlebyqq($v['qq'])['moneyadd'];
			$objExcel->getActiveSheet()->setCellValue('a'.$u1, $v["id"]);
			$objExcel->getActiveSheet()->setCellValue('b'.$u1, 'qq号'.$v["qq"]);
			$objExcel->getActiveSheet()->setCellValue('c'.$u1, $logins);
			$objExcel->getActiveSheet()->setCellValue('d'.$u1, date('Y-m-d H:i:s',$v["c_time"]));
			$objExcel->getActiveSheet()->setCellValue('e'.$u1, $v["platform"]);
			$objExcel->getActiveSheet()->setCellValue('g'.$u1, $moneyadd);
			$objExcel->getActiveSheet()->setCellValue('i'.$u1, $v["newmoneyadd"]);
			$i++;
		}
		...
		$objExcel->getActiveSheet()->getColumnDimension('C')->setWidth(5);
		...
		$objExcel->getActiveSheet()->getColumnDimension('I')->setWidth(50);
